...
class px(nn.Module): # 这个是在反卷积，上采样的。把所有的整到一起，做重建
...
class pzd(nn.Module): # 分类器，域的
    ...
    def forward(self, d):
        ...
        return zd_loc, zd_scale # 计算loss的时候，会反复调用，1个epoch64次

class pzy(nn.Module): # # 分类器，任务的
    def __init__(self, d_dim, x_dim, y_dim, zd_dim, zx_dim, zy_dim, args):
...
# Encoders
class qzd(nn.Module): # 下采样，域的。TODO 可以加BN
    def __init__(self, d_dim, x_dim, y_dim, zd_dim, zx_dim, zy_dim, args):
        ...
        self.fc11 = nn.Sequential(nn.Linear(640, zd_dim)) # TODO 这里怎么直接接预测分类了？
    ...
    def forward(self, x):
        ...
        zd_scale = self.fc12(out) + 1e-7 # TODO 是获得因子？
...
class qzx(nn.Module):
    def __init__(self, d_dim, x_dim, y_dim, zd_dim, zx_dim, zy_dim, args):
        ...
        self.fc11 = nn.Sequential(nn.Linear(640, zx_dim)) # 这个是域的
        self.fc12 = nn.Sequential(nn.Linear(640, zx_dim), nn.Softplus()) # 这个是生成任务 的 # TODO 看一下Softplus是多少
...
class GILE(nn.Module):
    def __init__(self, args):
        ...
        self.d_dim = args.n_domains #  看这个变量。是6，实际应该是4？
        self.x_dim = args.x_dim # input size after flattening? 1152
        self.y_dim = args.n_class # 17

        self.start_zx = self.zd_dim # 潜在是 50
        self.start_zy = self.zd_dim + self.zx_dim # 域的+任务的？
...
# 搞清楚p，q是啥，为什么有3个？
        ...
        self.aux_loss_multiplier_y = args.aux_loss_multiplier_y # multiplier for y classifier
        self.aux_loss_multiplier_d = args.aux_loss_multiplier_d #

        self.beta_d = args.beta_d # multiplier for KL d
    ...
    def forward(self, d, x, y):
        ...
        # Reparameterization trick 域的，p和q分别是？ TODO 获得重采样？是看分布吗？
        qzd = dist.Normal(zd_q_loc, zd_q_scale) # todo loc和scale是什么？
        zd_q = qzd.rsample()
        if self.zx_dim != 0:
        ...
        # Decode 3 也就是图中最后的地方； 分别是d，x和y的
        x_recon = self.px(zd_q, zx_q, zy_q, idxs_y, sizes_y)

        zd_p_loc, zd_p_scale = self.pzd(d) # 输入的d？ TODO 看尺寸，d的重构
        ...
        # Reparameterization trick 这难道也是最后的？ TODO 也是一样，算分布 看看高斯分布是啥样的
    ...
    def loss_function_false(self, args, d, x, y=None):
        ...
        loss_classify_true = args.weight_true * (F.cross_entropy(pred_d, d, reduction='sum') + F.cross_entropy(pred_y, y, reduction='sum')) # weights for classifier true
        loss_classify_false = args.weight_false * (F.cross_entropy(pred_d_false, d, reduction='sum') + F.cross_entropy(pred_y_false, y, reduction='sum')) # weights for classifier false
    ...
    def loss_function(self, d, x, y=None):
        ...
        x = torch.unsqueeze(x, 1)
        CE_x = F.mse_loss(x_recon, x.float()) #这里计算loss
    ...
    def classifier(self, x): # 这里是最后的分类任务。包括d和y的。
        """
    ...
    def get_features(self, x): # y的潜在向量
        zy_q_loc, zy_q_scale, idxs_y, sizes_y = self.qzy(x)
        qzy = dist.Normal(zy_q_loc, zy_q_scale)
        zy_q = qzy.rsample()
        return zy_q
